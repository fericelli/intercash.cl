		...
		private function retorno(){
            try{
				...
				if(isset($_POST["moneda"])){
					...
					$api = json_decode(file_get_contents("https://localbitcoins.com/api/equation/USD_in_".$_POST["moneda"]))->{'data'};
					...
					$api = json_decode(file_get_contents("https://localbitcoins.com/api/equation/USD_in_usd"))->{'data'};
                ...
                while($tasa = $this->Conexion->Recorrido($consulta)){
					...
					if($api!=""){
						$usdorigen =  number_format(json_decode(file_get_contents("https://localbitcoins.com/api/equation/USD_in_".$tasa["iso_moneda"]))->{'data'},$tasa["decimalesmoneda"],".","");
						$usdtorigen = number_format($usdorigen*floatval("1.".str_replace(".","",$tasa["porcentajedeva"])),$tasa["decimalesmoneda"],".","");
					}else{
						$usdorigen = $tasa["tasaoficial"];
                ...
                if($tasa = $this->Conexion->Recorrido($consulta)){
					...
					while($tasas = $this->Conexion->Recorrido($consultar)){
                        $cantidad ++;
						if($api!=""){
							$usddestino =  number_format(json_decode(file_get_contents("https://localbitcoins.com/api/equation/USD_in_".$tasas["monedaventa"]))->{'data'},$tasas["decimalesmoneda"],".","");
							$usdtdestino = number_format($usddestino*floatval("1.".str_replace(".","",strval($tasas["porcentaje"]))),$tasas["decimalesmoneda"],".","");
						}else{
							$usdorigen = number_format($tasas["tasaoficial"],$tasas["decimalesmoneda"],".","");
							$usdtorigen = number_format($this->Conexion->Recorrido($this->Conexion->Consultar("SELECT AVG(anunciocompra) FROM tasas WHERE monedacompra='".$tasas["monedaventa"]."' AND paisorigen='".$tasas["iso2"]."'"))[0],$tasas["decimalesmoneda"],".","");
